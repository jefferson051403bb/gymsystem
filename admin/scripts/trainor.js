...
function add_trainor()
{
    ...
    xhr.send(data);
        ...
        function add_image() {
            let data = new FormData();
            data.append('image', add_image_form.elements['image'].files[0]); // Correctly append the file
            data.append('trainor_id', add_image_form.elements['trainor_id'].value);
            data.append('add_image', ''); // Key to trigger the PHP handler
            ...
            xhr.onload = function () {
                if (this.responseText === 'inv_img') {
                    alert('Only JPG, WEBP, or PNG images are allowed!', 'image-alert');
                } else if (this.responseText === 'inv_size') {
                    alert('Image size should be less than 2MB!', 'image-alert');
                } else if (this.responseText === 'upd_failed') {
                    alert('Image upload failed. Server Down!', 'image-alert');
                } else if (this.responseText === 'success') {
                    alert('Image added successfully!', 'image-alert');
                    trainor_images(add_image_form.elements['trainor_id'].value, document.querySelector("#trainor-images .modal-title").innerText);
                    add_image_form.reset();
                } else {
                    alert('Unexpected response from server.', 'image-alert');
                }
            };
            xhr.send(data);
        }
